...
const CategoryForm: React.FC<CategoryFormProps> = ({ categoryId, onClose }) => {
	...
	const { items, operations, createNewCategory, updateExistingCategory, deleteCategoryById } =
		useCategories();
	const { user } = useAuth();
	...
	useEffect(() => {
		if (categoryId) {
          ...
          previewTitle: category.previewTitle || '',
					...
					hasSquareContent: category.hasSquareContent,
	...
	const handleSubmit = async (e: React.FormEvent) => {
        ...
        previewTitle: formData.previewTitle,
				...
				hasSquareContent: formData.hasSquareContent,
      ...
      <div>
			...
			<div className='flex flex-col sm:flex-row gap-4'>
				...
				<div className='flex items-center'>
					<input
						id='hasSquareContent'
						name='hasSquareContent'
						type='checkbox'
						checked={formData.hasSquareContent}
						onChange={handleChange}
						className='mr-2'
					/>
					<label
						className={isDark ? 'text-dark-text-primary' : 'text-light-text-primary'}
						htmlFor='hasSquareContent'
					>
						Category content is square
					</label>
				</div>
			</div>
			...
			<div className='flex justify-center mt-6 gap-3 '>
				...
				{isUpdateMode && (
					<ButtonBase onClick={handleDeleteCategory} className=' font-semibold' variant='ghost'>
						Delete Category
					</ButtonBase>
				)}
			</div>
